...
# 设置一些常量
################################################################################################
g = np.array([0, 0, -9.81])  # gravity：重力
l_jac = np.zeros([9, 6])
l_jac[3:, :] = np.eye(6)  # motion model noise jacobian：运动模型噪声的雅各比
h_jac = np.zeros([3, 9])
h_jac[:, :3] = np.eye(3)  # measurement model jacobian：测量模型的雅各比
...
# 设置卡尔曼滤波器的初值
################################################################################################
p_est = np.zeros([imu_f.data.shape[0], 3])  # position estimates                        (Nx3)
v_est = np.zeros([imu_f.data.shape[0], 3])  # velocity estimates                        (Nx3)
q_est = np.zeros([imu_f.data.shape[0], 4])  # orientation estimates as quaternions      (Nx4)
p_cov = np.zeros([imu_f.data.shape[0], 9, 9])  # covariance matrices at each timestep   (Nx[9x9])
...
p_cov[0] = np.zeros(9)  # covariance of estimate：估计的协方差
gnss_i  = 0
lidar_i = 0
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    v_hat = v_check + v_del
    # 这里需要注意global误差使用左乘
    q_hat = Quaternion(euler=phi_del).quat_mult_left(q_check,out = 'Quaternion').normalize().to_numpy() 
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + delta_t*v_est[k-1] + (delta_t**2 / 2)*(rotation_matrix.dot((imu_f.data[k-1] + imu_f.data[k])/2) + g)
    v_est[k] = v_est[k-1] + delta_t*(rotation_matrix.dot(imu_f.data[k-1]) + g)
    q_est[k] = Quaternion(axis_angle=((imu_w.data[k-1] + imu_w.data[k])/2) * delta_t).quat_mult_right(q_est[k-1],out = 'Quaternion').normalize().to_numpy()

    # 1.1 Linearize the motion model and compute Jacobians
    F = np.identity(9)
    Q = np.identity(6)
    F[:3, 3:6] = delta_t * np.identity(3)
    # 注意，这里是global误差下的表达形式，与local误差下的表达方式不同
    F[3:6, 6:] = -skew_symmetric(rotation_matrix.dot((imu_f.data[k-1] + imu_f.data[k])/2))*delta_t
